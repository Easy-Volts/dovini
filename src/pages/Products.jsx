...
const Products = () => {
  ...
  const [backendCategoryInfo, setBackendCategoryInfo] = useState(null);
  ...
  const fetchBackendProducts = useCallback(async (categoryId) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://api.dovinigears.ng/products/by-category?category_id=${categoryId}`
      );
      ...
      // Fetch category info from categories endpoint
      try {
        const categoryResponse = await fetch(
          "https://api.dovinigears.ng/categories"
        );
        ...
        console.log("Failed to fetch category info:", categoryError);
      }
    } catch {
      setIsUsingBackend(false);
      setBackendProducts([]);
  ...
  useEffect(() => {
    if (backendCategoryId && !isLoadingBackend && !isUsingBackend) {
      ...
      const category = categories.find(
        (cat) => cat.id === parseInt(backendCategoryId)
      );
      if (category) {
  ...
  const filteredAndSortedProducts = useMemo(() => {
    ...
    switch (sortBy) {
      ...
      default:
        filtered.sort(
          (a, b) => (b.isFlashDeal ? 1 : 0) - (a.isFlashDeal ? 1 : 0)
        );
    }
  ...
  const getCategoryInfo = () => {
    ...
    if (limitedStockParam) {
      return {
        name: "Limited Stock",
        description: "Almost gone - secure these items before they're sold out",
      };
    }

    if (isUsingBackend && backendCategoryId && backendCategoryInfo) {
      return backendCategoryInfo;
    }

    // Fall back to local category array for backend categories
      ...
      if (category) {
        return {
          ...
          description:
            ...
            `Premium ${category.name.toLowerCase()} equipment from our collection`,
        };
      }
    }
        ...
        <div className="container mx-auto px-4 py-6">
          ...
          >
            ...
            <p className="md:text-xl text-gray-600 max-w-2xl mx-auto">
              {flashDealsParam
                ? "Limited time offers with amazing discounts - don't miss out!"
                : limitedStockParam
                ? "Almost gone - secure these items before they're sold out!"
                : categoryInfo
                ? categoryInfo.description ||
                  `Discover our premium ${categoryInfo.name.toLowerCase()} equipment for professional photography`
                : searchQuery
